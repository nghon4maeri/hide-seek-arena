"""agent.py — Merged submission (Lab 2 Blind Adversary).

PacmanAgent (Seeker): ported from submissions/agent — interception,
                      Bayesian belief, frontier exploration, speed packing.
GhostAgent  (Hider) : ported from submissions/24127192 — hardcoded-map
                      topology cache, ensemble pacman prediction, risk
                      engine, Monte Carlo rollouts, policy portfolio.
"""
...
from ag_pathfinding import (
    DIRS as ag_DIRS,
    _cell as ag_cell,
    _apply as ag_apply,
    _manhattan as ag_manhattan,
    _cell_exits as ag_cell_exits,
    _legal as ag_legal,
    astar as ag_astar,
)
from ag_topology import TopologyAnalyzer as AgentTopoAnalyzer
from ag_belief_state import BeliefState as AgentBelief
...
class PacmanAgent(BasePacmanAgent):
    def __init__(self, **kwargs):
        ...
        self.belief = AgentBelief(21, 21)
        self.topo = AgentTopoAnalyzer()
        self.last_seen = None
    ...
    def _delta_to_move(self, delta):
        return MOVE.get(delta, Move.STAY)
    ...
    def _intercept(self, ms, enemy_pos):
        ...
        for i in range(2, 6):
            ...
            if not (0 <= nr < H and 0 <= nc < W):
                break
            if ag_cell(ms, nr, nc) == 1:
                break
            nxt = (nr, nc)
            exits = ag_cell_exits(nxt, ms)
            sc = exits * 200
        ...
        for r in range(H):
            for c in range(W):
                ...
                if sc > best_s:
                    best_s, best = sc, (r, c)
        if best:
            path = ag_astar(self.memory_map, me, best)
            if path:
                return self._pack_speed(path, me)
        moves = ag_legal(me, self.memory_map)
        if moves:
            d = min(moves, key=lambda d: self._visited.get(ag_apply(me, d), 0))
    ...
    def step(self, map_state, my_position, enemy_position, step_number):
        ...
        if self.topo.ready and me in self.topo.dead_ends:
            m = ag_legal(me, self.memory_map)
            if m:
                return MOVE.get(max(m, key=lambda d: self.topo.weight(
        ...
        if enemy is not None:
            ...
            if intercept:
                ...
                if path:
                    return self._pack_speed(path, me)
            path = ag_astar(self.memory_map, me, enemy)
            if path:
                return self._pack_speed(path, me)
