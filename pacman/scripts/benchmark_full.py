"""Full benchmark runner — runs N games with stochastic start and reports statistics.

Usage (from repo root):
    python pacman/scripts/benchmark_full.py --seek 24127457 --hide 24127192 --games 100

Output includes win/loss counts, win rates, and average steps for both sides.
"""
...
SUBMISSIONS_DIR = PACMAN_ROOT / "submissions"


def run_one_game(seek: str, hide: str, max_steps: int = 200,
                 capture_distance: int = 2, pacman_speed: int = 2) -> Dict:
    """Run a single game and return parsed result dict.

    Keys: winner_id, winner_role, total_steps, final_distance
    ...
    try:
        proc = subprocess.run(cmd, cwd=PACMAN_ROOT / "src", env=env,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Full benchmark — run N games with stochastic start and report statistics."
    )
    ...
    print(f"{'='*60}")
    print(f"  BENCHMARK: {args.seek} (Seeker) vs {args.hide} (Hider)")
    print(f"  Games: {args.games}  |  Max steps: {args.max_steps}")
    print(f"  Capture distance: {args.capture_distance}  |  Pacman speed: {args.pacman_speed}")
    print(f"  Start mode: stochastic")
    print(f"{'='*60}\n")
    ...
    if valid:
        ...
        # tie-break diff = avg_steps_seeker - avg_steps_hider (lower is better)
        if seek_steps:
    ...
    if errors:
        print(f"\n  Errors occurred ({err_count} games):")
        for e in errors[:5]:
            print(f"    - {e['error']}")
        if len(errors) > 5:
